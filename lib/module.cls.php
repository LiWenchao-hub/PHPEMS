...
class module
{
	...
	public function getMoudleFields($moduleid,$userinfo = 1,$app)
	{
		if(!$app)$app = $this->G->app;
		if($userinfo == 1)
		...
		$data = array(false,'module_fields',array(array('AND',"fieldmoduleid = :moduleid",'moduleid',$moduleid), array('OR'," (fieldpublic = 1 AND fieldappid = :app)",'app',$app),array('AND',"fieldlock = '0'")),false,'fieldsequence DESC,fieldid DESC');
		$sql = $this->pdosql->makeSelect($data);
	...
	//整理模型所需的参数，除去多余参数
	public function tidyNeedFieldsPars($args,$moduleid,$userinfo,$app)
	{
		$tmp = array();
		...
		$r = $this->getMoudleFields($moduleid,$userinfo,$app);
