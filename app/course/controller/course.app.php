	...
	private function endstatus()
    {
        ...
        if($r)
        {
            ...
            if($content)
			{
				...
				{
					$this->progress->modifyProgress($id,array('prscoursestatus' => 1,'prspersent' => 100));
				}
				else
				{
					if($passednumber > 0)
					{
						$per = intval(100 * $passednumber/$allnumber);
                        $this->progress->modifyProgress($id,array('prspersent' => $per));
					}
				}
			}
        }
...
//////////  如果没有登陆，请先登录   /////////////////////
		...
		$contents = $this->content->getCourseList(array(array("AND","coursecsid = :coursecsid",'coursecsid',$csid)),$page,15);
