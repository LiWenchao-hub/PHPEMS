...
class basic_exam
{
	...
	{
		$data = array("count(*) as number",array('openbasics','user'),array(array("AND","openbasics.obuserid = user.userid"),array("AND","obbasicid = :obbasicid",'obbasicid',$basicid),array("AND","obendtime >= :obendtime",'obendtime',TIME)));
		$sql = $this->pdosql->makeSelect($data);
		$r = $this->db->fetch($sql);
		return $r['number'];
	}
