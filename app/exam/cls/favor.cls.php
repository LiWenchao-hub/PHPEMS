...
class favor_exam
{
	...
	public function getExamHistoryListByArgs($page,$number = 20,$args = array(),$fields = false)
	{
		...
		$data = array($fields,array('examhistory','user'),$args,false,"ehusername DESC,ehid DESC",array(intval($page-1)*$number,$number));
		$sql = $this->pdosql->makeSelect($data);
		/*
		echo("<pre>");
		var_dump($sql);
	...
	//根据用户和科目获取考试记录列表
	public function getAllExamHistoryByArgs($args = array(),$fields = false,$orderby = 'ehscore DESC')
	{
		$args[] = array("AND","examhistory.ehuserid = user.userid");
		$data = array($fields,array('examhistory','user'),$args,false,$orderby,false);
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetchAll($sql);
	}
