...
class exam_exam
{
    ...
    //得到全部数据
    public function getQuestionsAllList($args = 1, $offset, $pageSize)
    {
        $r = array();
        $data = array('DISTINCT questions.*', array('questions', 'quest2knows'), $args, false, 'questions.questionid DESC', array($offset, $pageSize));
        $sql = $this->pdosql->makeSelect($data);
        $r['rows'] = $this->db->fetchAll($sql, false, array('questionhtml', 'questionknowsid'));
        $data = array('count(DISTINCT questions.questionid) AS number', array('questions', 'quest2knows'), $args);
        $sql = $this->pdosql->makeSelect($data);
        $t = $this->db->fetch($sql);
        $r['total'] = $t['number'];
