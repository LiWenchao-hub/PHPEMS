...
//        $fname = 'data/score/'.TIME.'-'.$userid.'-score.csv';
        ...
        $objSheet->getColumnDimension('B')->setWidth(20);//设置列宽度
        $objSheet->getColumnDimension('C')->setWidth(10);//设置列宽度
        $objSheet->getColumnDimension('D')->setWidth(15);//设置列宽度
...
//        foreach($rs as $p)
//        {
//            $tmp = array('usertruename' => iconv("UTF-8","GBK",$p['usertruename']),'normal_schID' => 'NO.'.iconv("UTF-8","GBK",$p['normal_schID']),'ehusername' => 'ID.'.iconv("UTF-8","GBK",$p['ehusername']),'ehexam' => iconv("UTF-8","GBK",$p['ehexam']),'ehscore' => $p['ehscore']);
//            $tmp['ehstarttime'] = date('Y-m-d H:i:s',$p['ehstarttime']);
