...
function ckplayerConfig() {
	return {
		...
		config: {
			...
			schedule: 3, //是否启用进度调节栏,0不启用，1是启用，2是只能前进（向右拖动），3是只能后退，4是只能前进但能回到第一次拖动时的位置，5是看过的地方可以随意拖动
			...
			buttonMode: {
				...
				timeSchedule: false, //鼠标在时间进度条上是否显示可点击形态
			...
			var keyDown = function(eve) {
				...
				switch(keycode) {
					...
					case 37:
						...
						thisTemp.videoSeek(now < 0 ? 0 : now);
						break;
					case 39:
						now = thisTemp.time + 10;
						thisTemp.videoSeek(now);
						break;
					case 38:
